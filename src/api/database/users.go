...
func (u *User) getUser(auth routers.AuthLevel) *res.ServerError {
	var serr res.ServerError
	if auth > routers.USER { // deleted search check
...
func (u *User) GetUserByName(auth routers.AuthLevel) *res.ServerError {
	var serr res.ServerError
...
func (u *User) GetUserByEmail(auth routers.AuthLevel) *res.ServerError {
	var serr res.ServerError
...
func (u *User) updateUser(auth routers.AuthLevel) *res.ServerError {
	var serr res.ServerError
	...
	if u.Password != nil && (auth == routers.USER || auth == routers.ADMINUSER || auth == routers.SERVER) {
		serr.Query += " password=?,"
		serr.Args = append(serr.Args, u.Password) // hash handled in handler
	}
	u.Password = nil // never return password in payload
	if u.Email != nil && (auth == routers.USER || auth == routers.ADMINUSER || auth == routers.SERVER) {
		serr.Query += " email=?,"
		serr.Args = append(serr.Args, u.Email)
	} else {
		u.Email = nil
	}
	if u.Country != nil && (auth == routers.USER || auth == routers.ADMINUSER || auth == routers.SERVER) {
		serr.Query += " country=?,"
		serr.Args = append(serr.Args, u.Country)
	} else {
		u.Country = nil
	}
	if u.Locale != nil && (auth == routers.USER || auth == routers.ADMINUSER || auth == routers.SERVER) {
		serr.Query += " locale=?,"
		serr.Args = append(serr.Args, u.Locale)
	} else {
		u.Locale = nil
	}
	if u.Banned != nil && (auth == routers.ADMIN) {
		serr.Query += " banned=?,"
		serr.Args = append(serr.Args, u.Banned)
	} else {
		u.Banned = nil
	}
	if u.LastToken != nil && (auth == routers.SERVER) {
		serr.Query += " last_token=?,"
		serr.Args = append(serr.Args, u.LastToken)
	} else {
		u.LastToken = nil
	}
	if u.LastLogin != nil && (auth == routers.SERVER) {
		serr.Query += " last_login=?,"
		serr.Args = append(serr.Args, u.LastLogin)
	} else {
		u.LastLogin = nil
	}
	if u.LastIP != nil && (auth == routers.SERVER) {
		serr.Query += " last_ip=?,"
...
func getUsers(start, count int, auth routers.AuthLevel) (*UserList, *res.ServerError) {
	var serr res.ServerError
	var rows *sql.Rows
...
func (u *User) CleanDataRead(auth routers.AuthLevel) {
	switch auth {
	case routers.SERVER:
